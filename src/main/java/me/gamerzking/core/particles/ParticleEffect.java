package me.gamerzking.core.particles;
...
public enum ParticleEffect {
     ...
     */
    public void display(float offsetX, float offsetY, float offsetZ, float speed, int amount, Location center, ViewDistance range) throws ParticleVersionException, ParticleDataException, IllegalArgumentException {
        if (!isSupported()) {
        ...
        new ParticlePacket(this, offsetX, offsetY, offsetZ, speed, amount, range.getDistance() > 256, null).sendTo(center, range.getDistance());
    }
     ...
     * @see ParticlePacket#sendTo(Location, double)
     */
    public void display(Vector direction, float speed, Location center, ViewDistance range) throws ParticleVersionException, ParticleDataException, IllegalArgumentException {
        if (!isSupported()) {
            throw new ParticleVersionException("This particle effect is not supported by your server version");
        }
        if (hasProperty(ParticleProperty.REQUIRES_DATA)) {
        ...
        new ParticlePacket(this, direction, speed, range.getDistance() > 256, null).sendTo(center, range.getDistance());
    }
     ...
     * @see ParticlePacket#sendTo(Location, double)
     */
    public void display(ParticleColor color, Location center, ViewDistance range) throws ParticleVersionException, ParticleColorException {
        if (!isSupported()) {
            throw new ParticleVersionException("This particle effect is not supported by your server version");
        }
        ...
        new ParticlePacket(this, color, range.getDistance() > 256).sendTo(center, range.getDistance());
    }
     ...
     * @see ParticlePacket#sendTo(Location, double)
     */
    public void display(ParticleData data, float offsetX, float offsetY, float offsetZ, float speed, int amount, Location center, ViewDistance range) throws ParticleVersionException, ParticleDataException {
        if (!isSupported()) {
            throw new ParticleVersionException("This particle effect is not supported by your server version");
        }
        if (!hasProperty(ParticleProperty.REQUIRES_DATA)) {
        ...
        new ParticlePacket(this, offsetX, offsetY, offsetZ, speed, amount, range.getDistance() > 256, data).sendTo(center, range.getDistance());
    }
     ...
     * @see ParticlePacket#sendTo(Location, double)
     */
    public void display(ParticleData data, Vector direction, float speed, Location center, ViewDistance range) throws ParticleVersionException, ParticleDataException {
        if (!isSupported()) {
            throw new ParticleVersionException("This particle effect is not supported by your server version");
        }
        if (!hasProperty(ParticleProperty.REQUIRES_DATA)) {
        ...
        new ParticlePacket(this, direction, speed, range.getDistance() > 256, data).sendTo(center, range.getDistance());
    }
    ...
    public static final class ParticlePacket {
        ...
        public static void initialize() throws VersionIncompatibleException {
            ...
            try {
                ...
                version = Integer.parseInt(UtilReflection.PackageType.getServerVersion().split("_")[1]);

                if (version > 7) {
                    enumParticle = UtilReflection.PackageType.MINECRAFT_SERVER.getClass("EnumParticle");
                }

                Class<?> packetClass = UtilReflection.PackageType.MINECRAFT_SERVER.getClass(version < 7 ? "Packet63WorldParticles" : "PacketPlayOutWorldParticles");

                packetConstructor = UtilReflection.getConstructor(packetClass);
                getHandle = UtilReflection.getMethod("CraftPlayer", UtilReflection.PackageType.CRAFTBUKKIT_ENTITY, "getHandle");
                playerConnection = UtilReflection.getField("EntityPlayer", UtilReflection.PackageType.MINECRAFT_SERVER, false, "playerConnection");
                sendPacket = UtilReflection.getMethod(playerConnection.getType(), "sendPacket", UtilReflection.PackageType.MINECRAFT_SERVER.getClass("Packet"));
        ...
        private void initializePacket(Location center) throws PacketInstantiationException {
            ...
            try {
                ...
                if (version < 8) {
                    ...
                    UtilReflection.setValue(packet, true, "a", name);
                } else {
                    UtilReflection.setValue(packet, true, "a", enumParticle.getEnumConstants()[effect.getId()]);
                    UtilReflection.setValue(packet, true, "j", longDistance);
                    if (data != null) {
                        int[] packetData = data.getPacketData();
                        UtilReflection.setValue(packet, true, "k", effect == ParticleEffect.ITEM_CRACK ? packetData : new int[] { packetData[0] | (packetData[1] << 12) });
                    }
                }
                UtilReflection.setValue(packet, true, "b", (float) center.getX());
                UtilReflection.setValue(packet, true, "c", (float) center.getY());
                UtilReflection.setValue(packet, true, "d", (float) center.getZ());
                UtilReflection.setValue(packet, true, "e", offsetX);
                UtilReflection.setValue(packet, true, "f", offsetY);
                UtilReflection.setValue(packet, true, "g", offsetZ);
                UtilReflection.setValue(packet, true, "h", speed);
                UtilReflection.setValue(packet, true, "i", amount);
            } catch (Exception exception) {
        ...
        public void sendTo(Location center, double range) throws IllegalArgumentException {
            ...
            center.getWorld().getPlayers().stream().filter(player -> player.getLocation().distanceSquared(center) <= squared).forEach(player -> sendTo(center, player));
        }
